...
def mpc_optimization(current_position, target_position, horizon=10, delta_t=0.05):
    """
    ...
    # maxA = np.array([3.5, 2.5, 3.0, 3.0, 3.0, 3.0, 3.0])
    # maxV = np.array([1.48, 1.13, 1.66, 1.66, 2.96, 2.96, 3.86])
    maxA = np.array([3.5, 2.5, 3.0, 3.0, 3.0, 3.0, 3.0])*0.75
    maxV = np.array([1.48, 1.13, 1.66, 1.66, 2.96, 2.96, 3.86])
    ...
    for k in range(horizon):
        ...
        # constraints += [U[:, k] >= -1.4, U[:, k] <= 1.4]  # General velocity limits
    ...
    # Solve the problem    while 'goal_position' not in globals():
    problem.solve()
    print("MPC_MPC_lol")
    return U.value  # Return the first velocity command


def plot_joint_data(time_data, joint_states, joint_velocities, joint_names, joint_goal):
    """
    ...
    plt.subplot(3, 1, 2)
    for joint in joint_names:
        plt.step(time_data, joint_velocities[joint], where='post', label=joint)
    ...
    for idx, joint in enumerate(limb.joint_names()):
        error = joint_goal[idx] - joint_states[joint]  # Calculate error
        plt.plot(time_data, error, label=f"{joint} Error")  # Plot error
    ...
    # Plot open loop solution at each point vs closed loop solution we calculate
...
def ik_service_client(goal_pos, *args, **kwargs):
        ...
        if (response.result_type[0] > 0):
            # rospy.loginfo("SUCCESS!")for idx, joint in enumerate(limb.joint_names()):
...
def main():
    ...
    rate = rospy.Rate(20)  # 10 Hz control loop
    ...
    start_time_0 = rospy.get_time()

    # goal_position = [ 0.66520896, -0.4393797, 0.37887031]
    ...
    current_position = np.array([-0.0003447265625, -0.9987998046875, -0.0013974609375,
                                  ...
                                  1.6983779296875])
    current_position = np.array([0, -1.5, 0, 2.0, 0, -0.5, 1.7])
    # target_position = np.array([-0.5, -1.5, -0.2, 1.5, -0.05, -1, 1])
    # target_position = np.array([-1.04436042, -0.99557957, 0.44128561, 1.35058592, -1.40465648, -0.65901033, 2.81789116])
    ...
    while not rospy.is_shutdown():
        ...
        if np.linalg.norm(current_state - target_position) < 0.2:
            joint_command = dict(zip(limb.joint_names(), np.array([0, 0, 0, 0, 0, 0, 0])))
            limb.set_joint_velocities(joint_command)
            rospy.loginfo("Target reached!")
            rospy.loginfo(f"Control Time Elapsed: {current_time}")
            rospy.loginfo(f"Total Time Elapsed: {rospy.get_time() - start_time_0}")
    ...
    plot_joint_data(time_data, joint_states, joint_velocities, limb.joint_names(), target_pos)
